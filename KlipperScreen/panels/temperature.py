...
class Panel(ScreenPanel):
    ...
    def translate_device_name(self, device):
        translations = {
            "extruder": "Extrusor0",
            "extruder1": "Extrusor1",
            "heater_bed": "Cama"
    ...
    def add_device(self, device):
        ...
        # Traducir el nombre del dispositivo
        translated_name = self.translate_device_name(devname)
        if translated_name != devname:
            logging.info(f"Device '{device}' translated to '{translated_name}'")
        ...
        if device.startswith("extruder"):
            ...
            image = "bed"
            devname = "Heater Bed"
            translated_name = self.translate_device_name(devname)  # Traduccion adicional
            if translated_name != devname:
                logging.info(f"Device 'Heater Bed' translated to '{translated_name}'")
        ...
        # Crear boton con el nombre traducido
        name = self._gtk.Button(
            image, self.prettify(translated_name), None, self.bts, Gtk.PositionType.LEFT, 1
        )
        ...
        # Gestionar manejadores de señales
        if self._printer.device_has_target(device):
            # Si ya existe un manejador para este dispositivo, desconectalo
            if device in self.long_press:
                existing_gesture = self.long_press.pop(device, None)
                if existing_gesture:
                    existing_gesture.disconnect_by_func(self.name_long_press)
                    existing_gesture.disconnect_by_func(self.name_long_press_cancelled)
    
            # Crear nuevos manejadores de senal
