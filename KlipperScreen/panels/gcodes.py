...
# Static buffer to store the last known values of the variables
VARIABLE_BUFFER = {
    "variable_enable_bed_mesh": 1,  # Default: enabled
    "variable_enable_purge_clean": 1,  # Default: enabled
    "variable_turn_off_when_finish": 0,  # Default: disabled
    "variable_enable_dual_toolchange": 0  # Default: disabled (synchronized toolchange)
}
...
class Panel(ScreenPanel):
    ...
    def confirm_print(self, widget, filename):
        ...
        # Layout for file preview and information
        label = Gtk.Label(
        ...
        # Load image and file info
        width = self._screen.width * .9 if self._screen.vertical_mode else self._screen.width * .5
        ...
        # Display dialog with file info and print options
    ...
    def confirm_print_response(self, dialog, response_id, filename):
        ...
        if response_id == Gtk.ResponseType.CANCEL:
            ...
            # Show the options panel after confirming "Print"
            self.show_variable_panel(filename)
        elif response_id == Gtk.ResponseType.REJECT:
            # Handle delete action
            self.confirm_delete_file(None, f"gcodes/{filename}")
    
    
    def show_variable_panel(self, filename):
        panel_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10, vexpand=True)
    
        # List of variables with user descriptions
        variable_list = [
            {"name": "variable_enable_bed_mesh", "description": "Ejecutar Bed Mesh al inicio"},
        ...
        # Create switches for each variable
        for var in variable_list:
            ...
            label.set_markup(f'<b>{var["description"]}</b>')
    
            switch = Gtk.Switch()
            switch.set_active(VARIABLE_BUFFER.get(var['name'], 0))
            switch.connect("notify::active", self.on_switch_toggled, var['name'])
            self.variables[var['name']] = switch
    
            row.pack_start(label, True, True, 5)
            row.pack_start(switch, False, False, 5)
            panel_box.pack_start(row, False, False, 10)
    
        buttons = [
            {"name": "Cancelar", "response": Gtk.ResponseType.CANCEL, "style": 'dialog-secondary'},
            {"name": "Imprimir", "response": Gtk.ResponseType.OK, "style": 'dialog-primary'}
        ]
    
        self._gtk.Dialog("Opciones de Impresion", buttons, panel_box, self.variable_panel_response, filename)
    
    
    def on_switch_toggled(self, switch, gparam, variable_name):
        # Update buffer with the current value of the switch
        value = 1 if switch.get_active() else 0
        VARIABLE_BUFFER[variable_name] = value
    
        if variable_name == "variable_enable_dual_toolchange" and value == 1:
            self.variables["variable_enable_purge_clean"].set_active(False)
    
        if variable_name == "variable_enable_purge_clean" and value == 1:
            self.variables["variable_enable_dual_toolchange"].set_active(False)
    
    def variable_panel_response(self, dialog, response_id, filename):
        self._gtk.remove_dialog(dialog)
    
        if response_id == Gtk.ResponseType.CANCEL:
            return
        elif response_id == Gtk.ResponseType.OK:
            # Save all variable states and start the print
            for var_name, switch in self.variables.items():
                value = 1 if switch.get_active() else 0
                VARIABLE_BUFFER[var_name] = value
    ...
    def _save_variable(self, variable_name, value):
        # Handles the setting of variables and displays the executed commands
        if variable_name == "variable_enable_purge_clean":
            # Enable/disable both purge and clean options
            purge_command = f'SET_GCODE_VARIABLE MACRO=_PRINT_VARIABLE VARIABLE=enable_toolchange_purge VALUE={value}'
            clean_command = f'SET_GCODE_VARIABLE MACRO=_PRINT_VARIABLE VARIABLE=enable_toolchange_clean VALUE={value}'
            self._screen._ws.klippy.gcode_script(purge_command)
            self._screen._ws.klippy.gcode_script(clean_command)
            logging.info(f'Executed: {purge_command}')
            logging.info(f'Executed: {clean_command}')


        else:
            # For other variables
            macro_variable = variable_name.replace("variable_", "")
            command = f'SET_GCODE_VARIABLE MACRO=_PRINT_VARIABLE VARIABLE={macro_variable} VALUE={value}'
