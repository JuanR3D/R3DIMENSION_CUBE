...
class Panel(MenuPanel):
    ...
    # This function has been modified by me in order to change the name of "extruder", "extruder1" and "heater_bed" to "extrusor0", "extrusor1", and "cama"
    ...
    def add_device(self, device):
        ...
        # Obtener el estado de temperatura del dispositivo
        temperature = self._printer.get_stat(device, "temperature")
        if temperature is None:
            return False
    
        # Manejar nombres compuestos y traducir nombres
        devname = device.split()[1] if len(device.split()) > 1 else device
    
        # Excluir nombres que comiencen con '_'
        if devname.startswith("_"):
            logging.info(f"Device '{device}' suppressed because it starts with '_'")
            return False
        ...
        # Configuración gráfica y botones
        ...
        name = self._gtk.Button(image, self.prettify(translated_name), None, self.bts, Gtk.PositionType.LEFT, 1)
