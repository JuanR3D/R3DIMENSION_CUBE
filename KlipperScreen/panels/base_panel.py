...
class BasePanel(ScreenPanel):
        ...
        try:
            ...
            for device in devices:
                ...
                if device.startswith("extruder") and self.current_extruder is False:
                    ...
                    self.control['temp_box'].add(self.labels[f"{device}_box"])
                    n += 1
                # Añado el sensor de habitaculo
                elif device.startswith("heater") or device == "temperature_sensor Habitaculo":
                    self.control['temp_box'].add(self.labels[f"{device}_box"])
                    n += 1
    ...
    def process_update(self, action, data):
        ...
        # Actualizar y manejar extrusor T0 (lógica especial)
        if "extruder" in self.labels and "extruder" in data["toolhead"]:
            temp_t0 = self._printer.get_stat("extruder", "temperature")
            if temp_t0 is not None:
                self.labels["extruder"].set_label(f"T0: {temp_t0:.0f}°")
    
        # Actualizar y manejar extrusor T1 (lógica especial)
        if "extruder1" in self.labels and "extruder1" in data["toolhead"]:
            temp_t1 = self._printer.get_stat("extruder1", "temperature")
            if temp_t1 is not None:
                self.labels["extruder1"].set_label(f"T1: {temp_t1:.0f}°")
