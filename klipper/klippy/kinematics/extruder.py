...
class PrinterExtruder:
    def __init__(self, config, extruder_num):
        ...
        # Setup main hotend heater
        pheaters = self.printer.load_object(config, 'heaters')
        gcode_id = 'T%d' % (extruder_num,)
        self.heater = pheaters.setup_heater(config, gcode_id)
        
        # Setup auxiliary heater from heater_generic
        self.aux_heater = None
        aux_heater_name = config.get('aux_heater', None)
        if aux_heater_name:
            self.aux_heater = self.printer.lookup_object(aux_heater_name, None)
        self.aux_temp_offset = config.getfloat('aux_temp_offset', 0.0)
    ...
    def cmd_M104(self, gcmd, wait=False):
        # Set Extruder Temperature for both main and auxiliary heaters
        ...
        # Set temperature for main heater
        pheaters.set_temperature(extruder.get_heater(), temp, wait)
        
        # Set temperature for auxiliary heater using aux_temp_offset
        if extruder.aux_heater is not None:
            aux_temp = temp + extruder.aux_temp_offset
            pheaters.set_temperature(extruder.aux_heater, aux_temp, wait)
