...
class ControlBangBang:
    def __init__(self, heater, config):
        ...
        # Leer parametros especificos del archivo de configuracion
        self.contrast_sensor_name = config.get('contrast_sensor', '')
        self.contrast_temp_diff = config.getfloat('contrast_temp_diff', 5.0, above=0.)
        self.eval_temp_diff = config.getfloat('eval_temp_diff', 5.0, above=0.)
        self.temp_correction = config.getfloat('temp_correction', 0.0)
        
        
    def temperature_update(self, read_time, temp, target_temp):
        # Inicializacion del valor PWM
        pwm_value = 0.0

        # Evaluacion normal del max_delta
        if self.heating and temp >= target_temp + self.max_delta + self.temp_correction:
            self.heating = False
        ...
        # Logica especifica para heater_bed
        if self.heater.get_name() == "heater_bed" and self.contrast_sensor_name:
            # Acceso directo al sensor de contraste
            sensor_contrast = self.heater.printer.lookup_object(f"temperature_sensor {self.contrast_sensor_name}", None)
            if sensor_contrast:
                temp_contrast = sensor_contrast.get_temp(read_time)[0]
                # Evaluar condiciones adicionales
                if abs(temp - target_temp) < self.eval_temp_diff and temp_contrast >= temp + self.contrast_temp_diff:
                    pwm_value = 0.0

        # Aplicacion del valor PWM decidido
        self.heater.set_pwm(read_time, pwm_value)
